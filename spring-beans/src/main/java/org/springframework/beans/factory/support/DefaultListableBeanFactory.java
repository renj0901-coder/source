...
public class DefaultListableBeanFactory extends AbstractAutowireCapableBeanFactory
	...
	public void preInstantiateSingletons() throws BeansException {
		...
		for (String beanName : beanNames) {
			// 获取合并后的BeanDefinition  继承父BeanDefinition上的属性
			// 并存入mergedBeanDefinitions，后面操作创建bean都是通过mergedBeanDefinitions
			RootBeanDefinition bd = getMergedLocalBeanDefinition(beanName);
			// 不是抽象、是单例的、不是懒加载的才会在创建容器的时候创建bean都是通过
			if (!bd.isAbstract() && bd.isSingleton() && !bd.isLazyInit()) {
				// 判断是不是FactoryBean
				if (isFactoryBean(beanName)) {
					// 获取并创建FactoryBean对象（原对象） &beanName
					...
					if (bean instanceof FactoryBean) {
						...
						if (System.getSecurityManager() != null && factory instanceof SmartFactoryBean) {
							...
							// 实现了SmartFactoryBean才会在spring启动的时候创建getObject，否则调用getBean的时候创建
					...
					// 不是FactoryBean，创建单例Bean对象
					getBean(beanName);
				}
			}
		}

		// 所有的非懒加载单例Bean都创建完了后，看是否实现SmartInitializingSingleton，执行afterSingletonsInstantiated方法
		...
		try {
			...
			if (value != null) {
				if (value instanceof String) {
					// 字符串，占位符填充(${})  没找到直接把他当字符串注入
					String strVal = resolveEmbeddedValue((String) value);
					BeanDefinition bd = (beanName != null && containsBean(beanName) ?
							getMergedBeanDefinition(beanName) : null);
					// 解析Spring表达式(#{})，找value属性的作为名字取找bean
					value = evaluateBeanDefinitionString(strVal, bd);
				}
				// 类型转换器， 可以根据自定义的类型转换器，将@value中的字符串转成bean
			...
			if (matchingBeans.size() > 1) {
				// 根据类型找到了多个Bean，进一步筛选出某一个, @Primary/ @Priority-->优先级最高--->name
				// 若果还是筛选不到，最后根据名字看能不能找到 byName的实现
	...
	protected String determineAutowireCandidate(Map<String, Object> candidates, DependencyDescriptor descriptor) {
		...
		// 取优先级最高的Bean @Priority 注解  数字越小优先级高
		...
		// Fallback
		// 如果根据类型还是找出了多个，那么再根据名字筛选 byName注入的实现
